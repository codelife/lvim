...
lvim.builtin.which_key.mappings["sh"] = { "<cmd>Telescope search_history<cr>", "Search history" }
...
lvim.builtin.which_key.mappings["tt"] = { "<cmd>AerialToggle<cr>", "Code Navigate" }
...
lvim.plugins = {
	...
	{
		"nacro90/numb.nvim",
		event = "CmdlineEnter", -- 懒加载：进入命令行才启用
		lazy = true,
		config = function()
			require("numb").setup({
				show_numbers = true, -- 预览时显示行号
				show_cursorline = true, -- 预览行高亮
				hide_relativenumbers = true, -- 预览时关掉相对行号
				number_only = false, -- 只在纯数字时触发
				centered_peeking = true, -- 预览行居中
			})
		end,
	},
	...
	},
	{
		...
		config = function()
			require("aerial").setup({
				layout = {
					min_width = 30,
					max_width = { 40, 0.2 }, -- 最大宽度 40 或 20% 窗口
					placement = "right", -- 右侧显示
					default_direction = "prefer_right",
				},
				attach_mode = "global", -- 全局一个窗口
				close_automatic_events = {},
				open_automatic = false, -- 不自动打开
				highlight_on_jump = 300, -- 跳转后高亮
				ignore = {
